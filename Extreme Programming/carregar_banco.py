...
def main():
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS public.clientes (id BIGINT, desconhecido1 BIGINT, desconhecido2 BIGINT, telefone VARCHAR(24), apelido VARCHAR(128));")
    cur.execute("CREATE TABLE IF NOT EXISTS public.pagamentos (id_cliente BIGINT, data_pagamento DATE, desconhecido BIGINT, valor DECIMAL, hasBeenPaid boolean);")
